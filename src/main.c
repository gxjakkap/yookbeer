...
void getCourseName(char name[], int course){
    if (course == 0) strcpy(name, "REG");
    else if (course == 1) strcpy(name, "INTER");
    else if (course == 2) strcpy(name, "HDS");
    else if (course == 3) strcpy(name, "RC");
}
 ...
 */
void printResHeader(){
    printf("%-15s %-32s %-15s %-10s %-34s %-15s\n", "ID", "FULLNAME", "NICK", "COURSE", "EMAIL", "PHONE");
}
...
void printSearchResultLine(Student cur){
    ...
    printf("%-15s %-32s %-15s %-10s %-34s %-15s\n", cur.id, cur.name, cur.nick, courseName, cur.email, cur.phone);
}
...
int sortDataFile(int len){
    ...
    f = fopen(DATA_PATH, "w");
    for(int j = 0; j < len; j++){
        fprintf(f, "%s,%s,%s,%d,%s,%s\n", d[j].id, d[j].name, d[j].nick, d[j].course, d[j].email, d[j].phone);
    }
    fclose(f);
    return 0;
}
...
void searchById() {
    ...
    if (inplen != 11 && inplen != 4){
        ...
        while (fgets(line, sizeof(line), f)) {
            removeTrailingNewline(line);
            if (sscanf(line, "%[^,],%[^,],%[^,],%d,%[^,],%[^,]", cur.id, cur.name, cur.nick, &cur.course, cur.email, cur.phone) == 6) {
                if (inplen == 4){
                    char subId[5] = {cur.id[7], cur.id[8], cur.id[9], cur.id[10]};
                    if (strcmp(subId, inp) == 0){
                        if (m == 0) printResHeader();
...
int addStd(int *len){
    ...
    for(int j = 0; j < strlen(fnm); j++){
        if (fnm[j] >= 'a' && fnm[j] <= 'z') fnm[j] -= 32;
    }
    for(int j = 0; j < strlen(lnm); j++){
        if (lnm[j] >= 'a' && lnm[j] <= 'z') lnm[j] -= 32;
    }
    // endsection
    ...
    if (buffer[0] == 'y' || buffer[0] == 'Y'){
        ...
        if (f == NULL){
            printf("[ERR] Cannot open %s . Cancelling and returning to main menu...", DATA_PATH);
            free(x);
            return 1;
        }
        fprintf(f, "%s,%s,%s,%d,%s,%s\n", x->id, x->name, x->nick, x->course, x->email, x->phone);
        fclose(f);
...
int main(){
    ...
    while(1){
        ...
        else if (c == 'N') searchByNickName();
        else if (c == 'F') searchByFirstName();
        else if (c == 'C') allStdCount();
        else if (c == 'A') addStd(&len); // TODO: check fn return value for status
        else if (c == 'R') remStd(&len); // TODO: check fn return value for status
        else if (c == 'I') searchById();
        else {
